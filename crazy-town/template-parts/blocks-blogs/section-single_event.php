...
add_filter( 'gform_field_value_custom_field_data', 'populate_custom_field_data' );
function populate_custom_field_data( $value ) {
    $post_id = get_the_ID();
    $data_event = get_field( 'data', $post_id );
    return $data_event;
}

add_filter( 'gform_field_value_custom_field_location', 'populate_custom_field_location' );
function populate_custom_field_location( $value ) {
    $post_id = get_the_ID();
    $location_event = get_field( 'location', $post_id );
...
<?php if ( have_rows( 'blog_default_sections' ) ) : ?>
    <?php while ( have_rows('blog_default_sections' ) ) : the_row(); ?>
        <?php if ( get_row_layout() == 'event_custom_form' ) : ?>
